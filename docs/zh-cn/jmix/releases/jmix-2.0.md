...
![Jmix2.0 发布](./_media/jmix-2.0/jmix2_cover.png) {.center .size-8 .radius .shadow}
...
![](./_media/jmix-2.0/jmix2_1.png) {.center .size-8}

工作日历扩展组件的新UI：

![](./_media/jmix-2.0/jmix2_2.png) {.center .size-8 }

Email 历史记录UI：

![](./_media/jmix-2.0/jmix2_3.png) {.center .size-8}

报表设计器在低分辨率的屏幕中更简洁易用：

![](./_media/jmix-2.0/jmix2_4.png) {.center .size-8}

为了在大多数情况下提供简洁的UI，报表设计器中一个带区默认只有单一数据集。但是，如果勾选 *多数据集* 复选框，设计器将切换到更复杂的视图：

![](./_media/jmix-2.0/jmix2_5.png) {.center .size-8 .shadow}

## 通用过滤器组件

`genericFilter` 组件的功能已经完善，支持在设计时和运行时创建配置和自定义条件。运行时的配置和自定义条件可以交由最终用户管理：

![](./_media/jmix-2.0/jmix2_6.png) {.center .size-8}

## 代码编辑器组件

基于Ace编辑器的新 `codeEditor` 组件支持用户查看和编辑带有语法高亮显示的代码。代码编辑器还有一些高级功能，如不同的颜色主题：

![](./_media/jmix-2.0/jmix2_7.png) {.center .size-8 .shadow}
...
![](./_media/jmix-2.0/jmix2_8.png) {.center .size-4}
...
![](./_media/jmix-2.0/jmix2_9.png) {.center .size-8 .shadow}

还可以使用下面这个选项对bean和其他元素按包名分组：

![](./_media/jmix-2.0/jmix2_a.png) {.center .size-8 .shadow}
...
![](./_media/jmix-2.0/jmix2_b.png) {.center .size-8}
...
![](./_media/jmix-2.0/jmix2_c.png) {.center .size-8 .shadow}
