...
var kt = class {
  constructor() {
    ...
    const s = "message-container", e = document.getElementById(s);
    e ? this.containerElement = e : (this.containerElement = document.createElement("div"), this.containerElement.id = s, document.body.appendChild(this.containerElement));
  }
  pop(s, e = 2e3) {
    const r = document.createElement("div"), w = Date.now();
    return r.className = "message move-in", r.innerHTML = s, this.containerElement.appendChild(r), this.messageElements[w] = r, e > 0 && setTimeout(() => {
      this.close(w);
    }, e), w;
  }
  close(s) {
    if (s) {
      const e = this.messageElements[s];
      e.className = e.className.replace("move-in", ""), e.className += "move-out", e.addEventListener("animationend", () => {
        e.remove(), delete this.messageElements[s];
      });
    } else
      Object.keys(this.messageElements).forEach((e) => this.close(Number(e)));
  }
  destory() {
...
//# sourceMappingURL=chunk-ZYA2Y74L.js.map
